...
data_by_specific_date = opsd_daily.loc['2014-01-20':'2014-01-22']
# print(data_by_specific_date)
data_by_general_date = opsd_daily.loc['2012-02']
# print(data_by_general_date)


# 3. Visualizing time series data:
# 3.1. Use seaborn style defaults and set the default figure size
sns.set(rc={'figure.figsize': (11, 4)})
opsd_daily['Consumption'].plot(linewidth=0.5)
plt.show()
# 3.2 Plotting other columns as each plot in the same graph
...
for name, ax in zip(['Consumption', 'Solar', 'Wind'], axes):
    sns.boxplot(data=opsd_daily, x='Month', y=name, hue='Month', ax=ax,
                palette=sns.color_palette('husl', 12), legend=False)
    ax.set_ylabel('GWh')
    ax.set_title(name)
...
# 5. Frequencies
data_by_freq = pd.date_range('1998-03-10', '1998-03-15', freq='D')
# print(data_by_freq)
...
consumption_sample = opsd_daily.loc[times_sample, ['Consumption']].copy()
# print(consumption_sample)

# Conver the data to daily frequency without filling any missings
consum_freq = consumption_sample.asfreq('D')
consum_freq['Consumption - Forward Fill'] = consumption_sample.asfreq(
    'D', method='ffill')
# print(consum_freq)
...
# The defaut index of the resampled DataFrame is the last day of each year, ('2006-12-31', '2007-12-31', ...) so to make life easier, set the index to the year component
